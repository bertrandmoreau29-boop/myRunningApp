...
def parse_fit_file(path: Path) -> dict[str, Any]:
    ...
    has_stryd_power = any(_positive_int(_get(record, "Power")) is not None for record in records)
    power_field_order = ("Power", "power") if has_stryd_power else ("power", "Power")
    ...
    summary = {
        ...
        "avg_power": avg_power,
        "max_power": (_positive_int(_get(session, "max_power")) if not has_stryd_power else None) or _max_int(record_powers),
    ...
    for index, lap in enumerate(laps, start=1):
        ...
        parsed_laps.append(
            ...
            "max_power": (_positive_int(_get(lap, "max_power")) if not has_stryd_power else None) or _max_int(lap_powers),
